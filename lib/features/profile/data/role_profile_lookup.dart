...
Future<RoleProfileModel> lookupRoleProfile(String role, String handle) async {
  ...
  try {
    // profiles lookup already done via /v1/social/profile above
  } catch (e) {
...
Future<Map<String, dynamic>?> _fetchEntity(
  ...
  try {
    ...
    for (final r in results) {
      if (r['role'] == roleKey || r['_kind'] == roleKey) return r;
      if ((r['handle'] as String? ?? '') == key) return r;
    }
    // Try profile lookup
    final p = await vps.get<Map<String,dynamic>>('/v1/social/profile/$key');
    return p.data?['user'] as Map<String,dynamic>?;
  } catch (_) {
    return null;
  }
}
...
Future<List<ProfilePost>> _loadPosts(dynamic vpsParam, String? uid) async {
  ...
  try {
    ...
    for (final r in res) {
      ...
      final tags = r['hashtags'];
      posts.add(ProfilePost(
        text: (r['content'] as String?) ?? '',
        hashtags: tags is List ? [for (final x in tags) x.toString()] : const [],
        timeAgo: _age(r['createdAt']?.toString()),
