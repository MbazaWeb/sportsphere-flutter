...
//     `approve_claim` itself also has an `is_app_admin()` guard.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (userErr || !userData?.user) {
      console.error("approve-claim: invalid token", userErr?.message);
      return json({ error: "Invalid or expired token" }, 401, req);
    }
    const callerUid = userData.user.id;

    // ── 2. Verify admin role (H9) ──────────────────────────────────────────
    // Prefer the DB-side is_app_admin() RPC (single source of truth).
    let isAdmin = false;
    try {
      const { data: adminFlag } = await supabase.rpc("is_app_admin");
      isAdmin = Boolean(adminFlag);
    } catch (e) {
      console.error("approve-claim: is_app_admin RPC failed", String(e));
    }

    // Defence-in-depth: also check the role column directly.
    if (!isAdmin) {
    ...
    if (error) {
      console.error("approve-claim: RPC error", error.message);
      return json({ error: error.message }, 400, req);
    }
    return json({ ok: true, data }, 200, req);
  } catch (e) {
    console.error("approve-claim error:", String(e));
